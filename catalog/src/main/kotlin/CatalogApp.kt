...
class ProductHandler(
    ...
    private suspend fun Product.withDetails(price: Price?, stocks: Array<InStockLevel>, recommendation: Recommendation): ProductWithDetails {
        val productRecommendations = recommendation.recommendationIds
            .mapNotNull { repository.findById(it.toLong()) }
            .map { ProductWithoutDetails(it.id, it.name, it.description) }
        ...
        fun Array<InStockLevel>.toArray() = map(InStockLevel::toOutStockLevel)
            ...
            .toTypedArray()
        return ProductWithDetails(name, description, price?.price ?: 0F, stocks.toArray(), productRecommendations.toTypedArray())
    }
}
